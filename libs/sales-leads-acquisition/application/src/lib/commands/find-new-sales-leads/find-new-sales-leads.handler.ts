...
import {SalesLead, SalesLeadData} from "@sales-leads/sales-leads-acquisition/domain";
...
export class FindNewSalesLeadsHandler
  ...
  constructor(
    ...
    private repository: SalesLeadsRepository
  ) {
  }

  async execute(command: FindNewSalesLeadsCommand): Promise<void> {
    const found = await Promise.allSettled(this.finders.map(finder => finder.find()));
    const results = found.filter((result): result is PromiseFulfilledResult<SalesLeadData[]> => result.status === 'fulfilled').map((result: PromiseFulfilledResult<SalesLeadData[]>) => result.value)
    const flatResults = results.flat()

    await Promise.allSettled(flatResults.map(result => {
      const salesLead = this.publisher.mergeObjectContext(SalesLead.acquired(result))
